...
function goal_wall_segment(wall::Int,room_w::Float64=5.0)
    ...
    if wall == 1
        segment = LineSegment([-20-room_w,0-room_w],[-20-room_w,0+room_w])
    elseif wall == 2
        segment = LineSegment([10,0+room_w],[10+room_w,0+room_w])
    elseif wall == 3
        segment = LineSegment([10+room_w,0+room_w],[10+room_w,0-room_w])
    elseif wall == 4
        segment = LineSegment([10,0-room_w],[10+room_w,0-room_w])
    elseif wall == 6
        segment = LineSegment([-20-room_w,-20],[-20+room_w,-20])
    end
    return segment
end
"""
...
#return length(float)
    ...
    if abs(p1[1]-p2[1])<MARGIN     #vertical line
        if abs(p1[1]-p0[1])<MARGIN  #p0 in line
            return 0.0
        end
...
#            @show "vertial headin"
        ...
        if abs(p1[2]-p0[2])<MARGIN  #p0 in line
            return 0.0
        end
...
#        @show "wrong way",ls,p0,heading
    ...
    if !segment_contact(ls,[x,y]) #destnation not in ls
